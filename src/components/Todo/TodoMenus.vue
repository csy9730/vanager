...
export default {
  ...
  computed: {
    todoList() {
      const number = this.$store.getters.gTodoList.length;
      if (this.$store.getters.gTodoList.length < this.todoNum) {
         this.goList(this.$store.getters.gTodoList[0].id);
      }
      // this.todoNum = number;
      return this.$store.getters.gTodoList; // 返回vuex getters.js 定义的getTodoList数据
    }
  },
