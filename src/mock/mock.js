...
export default {
  ...
  start() { // 初始化函数
    ...
    // 获取todo列表
    mock.onGet('/api/todo/list').reply(config => { //  config 指 前台传过来的值
    ...
    // 新增一条todo
    mock.onPost('/api/todo/addTodo').reply(config => {
      Todos.push({
        id: Mock.Random.guid(),
        title: 'newList',
        isDelete: false,
    ...
    // 获取todo单个列表
    mock.onGet('/api/todo/listId').reply(config => {
      let {
        id
      } = config.params;
      // id 是传进来的值
    ...
    // 修改标题
    mock.onPost('/api/todo/editTodo').reply(config => {
      let {
        todo
    ...
    // 新增一条代办事项
    mock.onPost('/api/todo/addRecord').reply(config => {
      let {
        id,
        text
      } = JSON.parse(config.data);
    ...
    // 修改标题
    mock.onPost('/api/todo/editRecord').reply(config => {
      let {
        id,
        record,
        index
